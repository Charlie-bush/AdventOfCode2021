...
public class Day11P2 implements AdventDay {
    ...
    public void process() {
        ...
        int steps = 0;
        boolean notSynchronisedFlash = true;

        while (notSynchronisedFlash) {
            ...
            resetBooleanArrays();
            steps++;
            if (isArrayEmpty(energyArray)) {
                notSynchronisedFlash=false;
            }
        }
        result = String.valueOf(steps);
    }

    private boolean isArrayEmpty(int[][] energyArray){
        for (int i = 0; i <10; i++) {
            for (int j= 0; j< 10; j++) {
